    ...
    return (
      <div className='container my-5'>
        <div className='empty-cart flex justify-center align-center flex-column font-manrope'>
          ...
          <Link to = "/" className='shopping-btn bg-black text-white fw-5'>Go shopping Now</Link>
        </div>
      </div>
    )
  }
              ...
              carts.map((cart, idx) => {
                return (
                  <div className='cart-ctr py-4' key = {cart?.id}>
                    ...
                    <div className='cart-ctd'>
                      <span className='cart-ctxt text-black fw-5'>{formatPrice(cart?.totalPrice)}</span>
                    </div>
          ...
          <div className='cart-cfoot flex align-start justify-between py-3 bg-white'>
            ...
            <div className='cart-cfoot-r flex flex-column justify-end'>
              <div className='total-txt flex align-center justify-end'>
                ...
                <span className='text-black fs-22 mx-2 fw-6'>{formatPrice(totalAmount)}</span>
              </div>

            <Link to="/checkout"> <button type = "button" className='checkout-btn  fs-16'>Check Out</button></Link> 
            </div>
          </div>
        </div>
      </div>
